...
--
{
 ...
 list-Of-Called-Asserted-Identity [42] ListOfInvolvedParties OPTIONAL, -- ATCF only
 ...
 *
 */
@ASNTag(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 82,constructed = true,lengthIndefinite = false)
public class IBCFRecord 
{
	...
	public IBCFRecord(RecordType recordType, boolean retransmission, String sipMethod,
			...
			List<InvolvedParty> listOfCalledAssertedIdentity,
			List<RealTimeTariffInformation> realTimeTariffInformation, String transitIOIList,
			...
			byte[] additionalAccessNetworkInformation, byte[] routeHeaderReceived,
			...
			List<AccessTransferInformation> listOfAccessTransferInformation, List<String> fEIdentifierList) 
	{
		if(recordType!=null)
		{
		...
		{
			this.initialIMSChargingIdentifier = new ASNOctetString();
			this.initialIMSChargingIdentifier.setValue(Unpooled.wrappedBuffer(initialIMSChargingIdentifier));
		}
	...
	public String toString() 
	{
        ...
        if(initialIMSChargingIdentifier!=null && initialIMSChargingIdentifier.getValue()!=null)
        {
	        ...
	        sb.append(ASNOctetString.printDataArr(getInitialIMSChargingIdentifier()));
	        sb.append("]");
        }
